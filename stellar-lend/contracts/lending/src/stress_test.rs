...
fn test_stress_zero_amount_operations() {
    ...
    client.borrow(&user, &asset, &min_amount, &collateral_asset, &(min_amount * 2));
...
fn test_stress_mixed_operations_large_user_base_with_additional_operations() {
    ...
    for user in users.iter().take(50) { // Test subset for performance
        // Repay some debt
...
fn test_stress_alternating_borrow_repay_cycles() {
    ...
    for i in 0..20 {
        ...
        // Borrow again
        client.borrow(&user, &asset, &(borrow_amount / 2), &collateral_asset, &(collateral_amount / 2));
    }
